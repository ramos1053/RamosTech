...
//             }
            ...
            CommandGroup(replacing: .undoRedo) {
                Button("Undo") {
                    NSApp.keyWindow?.firstResponder?.tryToPerform(Selector(("undo:")), with: nil)
                }
                .keyboardShortcut("z", modifiers: .command)

                Button("Redo") {
                    NSApp.keyWindow?.firstResponder?.tryToPerform(Selector(("redo:")), with: nil)
                }
            ...
            CommandMenu("Debug") {
                ...
                Menu("Log Level") {
                    ForEach(DebugLogger.LogLevel.allCases, id: \.self) { level in
                        Button(action: {
                            Task { @MainActor in
                                DebugLogger.shared.currentLogLevel = level
                            }
                        }) {
                            HStack {
                                Text("\(level.icon) \(level.displayName)")
                                if DebugLogger.shared.currentLogLevel == level {
                                    Spacer()
                ...
                Toggle("Enable Logging", isOn: Binding(
                    get: { DebugLogger.shared.isEnabled },
                    set: { newValue in
                        Task { @MainActor in
                ...
                Button("Export Logs...") {
                    if let url = DebugLogger.shared.exportLogs() {
                        NSWorkspace.shared.activateFileViewerSelecting([url])
                    }
                }

                Button("Open Log Directory") {
                    let url = DebugLogger.shared.getLogDirectory()
