...
final class SyntaxHighlighter: ObservableObject {
    ...
    func attributedString(for text: String, baseFont: NSFont, theme: EditorTheme) -> NSAttributedString {
        let tokens = tokenize(text: text)
        let attributedString = NSMutableAttributedString(string: text)

        // Set base attributes
        attributedString.addAttribute(.font, value: baseFont, range: NSRange(location: 0, length: text.count))
        attributedString.addAttribute(.foregroundColor, value: theme.textColor.nsColor, range: NSRange(location: 0, length: text.count))

        // Apply token colors
        for token in tokens {
            let nsRange = NSRange(location: token.range.lowerBound, length: token.range.count)
            attributedString.addAttribute(.foregroundColor, value: token.type.color(for: theme), range: nsRange)
        }
